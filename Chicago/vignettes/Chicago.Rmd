...
In this section, we describe the expected properties of the diagnostic plots. 

Note that the diagnostic plots above are computed on the fly for a small fraction of the full GM12878 dataset and using a small number of fragment pools in parameter estimation. The trends we describe below should generally be more pronounced with real-world, genome-wide datasets.

1. *Brownian noise other end factors*: The adjustment made to the mean Brownian noise count, estimated in the pools of other ends. ("tlb" refers to the number of trans-chromosomal reads that the other end accumulates in total. "B2B" stands for a "bait-to-bait" interactions).
...
- In the lower subplot, the bait-to-bait estimates (here, the five bars on the far right) should be higher, on average, than the others. Both groups should also have medians and variances that trend upwards, moving from left to right.
...
- The function should be monotonously decreasing.
...
Additionally, a set of precomputed weights are provided in the package:
