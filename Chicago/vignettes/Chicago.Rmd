...
WARNING: The data set used in this tutorial is only a small part of a real data set. Do not use any of the input data in your own analysis.

We will be releasing a companion data package that has a fully executable workflow.
...
Examples of these files are provided in the package, as follows. Do not use these files in your analysis!
...
2. You will also need your input data files, whose names typically end with *_distSign.txt*. These files can be obtained by running ...?

```{r}
testDataPath <- file.path(dataPath, "unitTestData")
files <- c(
    file.path(testDataPath, "unitTest_mESC_rep1_step1_distSign.txt"),
    file.path(testDataPath, "unitTest_mESC_rep2_step1_distSign.txt")
  )
files
```
