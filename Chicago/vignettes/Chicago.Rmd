...
From here, we can use standard Bioconductor functionality. In the following example, we use `r Biocpkg("AnnotationHub")` to fetch a relevant enhancer mark track (H3K4me1) from the ENCODE project, then find which other ends overlap with the enhancer mark.
...
Note that the annotation's genome version should match that of the promoter capture data, namely hg19:
```{r}
hs["AH23254"]$genome
```
