...
NOTE: A wrapper to perform this type of analysis, called *runChicago.R*, is provided as part of *chicagoTools* [FIXME URL].

WARNING: The data set used in this tutorial comes from the package *PCHiCdata*. This package contains a small part of the real data set (two chromosomes) from Schoenfelder et al [@Schoenfelder2015] (note that boths papers used a different interaction calling algorithm and we are only reusing raw data from them). Do not use any of these sample input data for purposes other than training - download full BAM files and restart the analysis from them instead. 
...
- *nperbin* file (.npb), *nbaitsperbin* file (.nbpb), proxOEfile (.poe) - Precompute these tables from the .rmap and .baitmap files, using the Python scripts from *chicagoTools* [FIXME URL]: ``countNperBin.py``, ``countNbaitsPerBin.py`` and ``getProxOE.py``. 

It is easiest to put all of these files in the same directory. If you are using HindIII restriction enzyme in mouse and human and the same captured baits as in [@Schoenfelder2015] (mouse) and [@Mifsud2015] (human), you can use the files on our website [FIXME URL], otherwise you will need to generate them yourself using ``chicagoTools`` scripts as mentioned above. Refer to ``chicagoTools`` README file for more details.

Examples of these files for a two-chromosome sample of the mESC data used in this vignette are also provided in the PCHiCdata package, as follows. Do not use these files in your analysis!
