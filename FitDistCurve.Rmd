...
like this (but log.p can be omitted if threshold <- NA):

> head(maxPvals)
>   baitID otherEndID distSign       log.p
> 1:    219        197   -88681 -0.08264367
> 2:    219        198   -82433 -0.31432547
> 3:    219        199   -76075 -0.24835447
> ...
...
keyword <- "Macrophage" ##only MACROPHAGES

rdaFile <- paste0("~/Projects/ChicagoPiRelFromOverlap/PvalSummary",keyword,".Rda")
message("Loading ", rdaFile)
load(rdaFile)

##coerce to data.table, if required
if(!"data.table" %in% class(maxPvals))
  {
    maxPvals <- data.table(maxPvals)
  }
```
...
plot(log(data.lr$mid), log(p.fit(log(data.lr$mid), outJack[[1]]$par)), type="l", ylim = c(-19,-4), col=rainbow(5)[1], main=paste0("Data subsetting (",keyword,")"), xlab="log_e(distance)", ylab="log_e(prior probability of interaction)")
for(i in 2:Nsub)
{
