...
summary(lm(log2normalised_expression~distance_to_nearest_bait+interacting, data=TRIP_S1_combined))


## DEPRECATED ##

setDT(TRIP_S1_combined)
fit <- aov(log2normalised_expression ~ interacting * dist.bin, data=TRIP_S1_combined)
summary(fit)
...
histogram <- ggplot(residuals , aes(x=residuals))
histogram + geom_histogram(aes(y=..density..), colour="black",fill="white") +
  labs(x="Studentized Residual",y="Density")+
  stat_function(fun=dnorm, args=list(mean=mean(residuals$residuals,na.rm=TRUE),
                                             sd=sd(residuals$residuals,na.rm=TRUE))
                        , colour="red",size=1)